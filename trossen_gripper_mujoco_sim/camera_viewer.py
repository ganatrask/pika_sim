#!/usr/bin/env python3
"""
Trossen Gripper Camera Viewer

Renders on-board and scene cameras during the pick-and-place demo.

Usage:
    python camera_viewer.py                          # All cameras
    python camera_viewer.py --cameras realsense_d405 # Wrist cam only
    python camera_viewer.py --depth                  # Include depth
    python camera_viewer.py --speed 0.5              # Slow motion
    python camera_viewer.py --record                 # Save to video
...
from pick_and_place import SmoothController, generate_waypoints, randomize_cube
...
def main():
    parser = argparse.ArgumentParser(description="Trossen Gripper Camera Viewer")
    ...
    print(f"\n=== Trossen Camera Viewer ===")
    print(f"  Cameras: {', '.join(cameras)}")
    print(f"  Resolution: {args.width}x{args.height}")
    print(f"  Depth: {'ON' if args.depth else 'OFF'}")
    print(f"  Speed: {args.speed:.1f}x")
    print(f"  Controls: [Q] quit | [SPACE] pause | [1-{len(cameras)}] toggle camera | [R] reset")
    print(f"{'=' * 30}\n")
    ...
    render_every = 33  # ~30fps at 0.001 timestep
    step_count = 0
    ...
    try:
        while True:
            ...
            if frames:
                ...
                cv2.imshow("Trossen Camera Viewer", composite)
