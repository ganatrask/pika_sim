#!/usr/bin/env python3
"""
Trossen Gripper Pick-and-Place Demo (Dynamic / Randomized)
...
# ============================================================
# Constants — Trossen gripper
# ============================================================

# Trossen gripper: 0 = closed, 0.044 = fully open
GRIPPER_OPEN = 0.044
GRIPPER_CLOSED = 0.0

# Gantry Z positions (slide_z range: -0.4 to 0)
# Trossen gripper pads are ~0.115m from link_6 origin (along local +X / world -Z)
# Mounting offset: -0.1m below gantry_z
# Pad world Z = 0.75 + z_slide - 0.1 - 0.115 = 0.535 + z_slide
# For pad at cube Z (0.22): z_slide = 0.22 - 0.535 = -0.315
GRASP_Z = -0.312             # Slide Z for pads at cube height
HOVER_Z = -0.20              # Well above table for transit
HOME_Z = -0.05

# Max finger gap when fully open (~88mm)
MAX_GAP = 0.088
...
class SensorLogger:
    ...
    def log(self, data):
        ...
        # Gripper: convert slide (m) to distance and normalized value
        # Trossen: joint=0 → closed (~4mm gap), joint=0.044 → open (~88mm gap)
        # Each carriage moves 0.044m outward, gap = base_gap + 2*joint_pos
        base_gap = 0.004  # ~4mm gap when closed (joint=0)
        grip_distance = base_gap + pos_grip * 2
        grip_distance = np.clip(grip_distance, 0.0, MAX_GAP)
        grip_normalized = 1.0 - grip_distance / MAX_GAP  # 0=open, 1=closed
        ...
        print(
            ...
            f"  | gripper: norm={grip_normalized:.3f} dist={grip_distance*1000:.1f}mm"
...
def main():
    parser = argparse.ArgumentParser(description="Trossen Gripper Pick and Place (Randomized)")
    ...
    print("\n=== Trossen Gripper Pick-and-Place (Randomized) ===")
    print(f"  Speed: {args.speed:.1f}x | Total time: {controller.total_time:.1f}s")
    print("=" * 55)
    print("  Logging: joints(pos/vel/frc) + gripper(norm/dist/vel/frc) + IMU(accel/gyro/quat) + cube_pos")
    print("-" * 55)
