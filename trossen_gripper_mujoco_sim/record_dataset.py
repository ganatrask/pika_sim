#!/usr/bin/env python3
"""
Record pick-and-place episodes to HDF5 dataset (Trossen Gripper).
...
Dataset structure per episode:
    ...
    │   ├── gripper_width    (T, 1) float32   # meters, 0.0-0.088
...
from pick_and_place import (
    ...
    GRIPPER_OPEN, GRIPPER_CLOSED, MAX_GAP,
)
...
class EpisodeRecorder:
    ...
    def _get_gripper_width(self, data):
        """Get gripper opening width in meters."""
        pos_grip = self._read_sensor(data, "gripper_sensor")[0]
        # Trossen: joint=0 → ~4mm gap, joint=0.044 → ~88mm gap
        base_gap = 0.004
    ...
    def _get_ee_pos(self, data):
        ...
        # Gantry base at (0, 0, 0.75), gripper body offset (0, 0, -0.1)
        world_x = x
        world_y = y
        world_z = 0.75 + z - 0.1
    ...
    def record_step(self, data, ctrl_xyz, ctrl_grip):
        ...
        # Trossen: 0.044=open, 0.0=closed → invert
        grip_normalized = np.clip(1.0 - ctrl_grip / 0.044, 0.0, 1.0)
...
def main():
    parser = argparse.ArgumentParser(description="Record Trossen gripper pick-and-place dataset")
    ...
    for ep in range(args.episodes):
        ...
        # Open gripper and let physics settle before recording
        data.ctrl[3] = 0.044  # GRIPPER_OPEN
        for _ in range(200):  # 200 steps × 1ms = 0.2s settle time
            mujoco.mj_step(model, data)
